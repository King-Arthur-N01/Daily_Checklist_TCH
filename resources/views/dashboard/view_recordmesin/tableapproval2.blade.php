            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="alert alert-info alert-dismissible fade show" role="alert">
                        ...
                        <i class="modal-alert">Data update failed. Record already aprrove by someone else.</i>
                        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#ExtralargeModal').on('shown.bs.modal', function(event) {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('fetchapproval', ':id') }}'.replace(':id', id),
            ...
            $('#saveButton').on('click', function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: '{{ route('pushapproval', ':id') }}'.replace(':id', machineId),
                    data: {
            ...
            $('#rejectButton').on('click', function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: '{{ route('pushreject2', ':id') }}'.replace(':id', machineId),
                    data: {
                        '_token': '{{ csrf_token() }}', // Include the CSRF token
                        'reject_by': rejectBy
                    },
